        ...
        if ret:
            ...
            for (x, y, l, a) in detected_faces:
                ...
                if name!="Unknown":    
                    addInfo_In(name)
                stu_new.append(name)
                cv2.rectangle(img, (y, x), (y1, x1), (200, 0, 200), 4)
                cv2.rectangle(img, (y0, x0 + 35), (y1, x0), (200, 0, 200), cv2.FILLED)
                font = cv2.FONT_HERSHEY_DUPLEX
                cv2.putText(img, name , (y0 + 6, x0 +25), font, 1.0, (255, 255, 255), 1)
                cv2.putText(img, str(round(distance_test,4)) , (y0 + 6, x0 -10), font, 1.0, (255, 0, 255), 1)
            for i in stu_old:
                if i not in stu_new:
                    addInfo_Out(i)
            stu_old=stu_new
