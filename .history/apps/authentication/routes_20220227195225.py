...
detector_faceTest = cv2.CascadeClassifier("haarcascade_frontalface_default.xml")
@blueprint.route('/')
def route_default():
...
def gen_frames():
    ...
    while camera.isOpened():
        ...
        if not success:
            ...
            image_grey=cv2.cvtColor(frame,cv2.COLOR_BGR2RGB)
            # with graph.as_default():
            bboxes = detector_faceTest.detectMultiScale(image_grey, scaleFactor=1.5, minSize=(30, 30))
