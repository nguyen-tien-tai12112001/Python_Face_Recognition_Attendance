...
from apps.authentication.models import Attendance, Attendances, ClassMember, Users,Students,Class
...
def VerifierStudent(student): 
    ...
    return kq
listStudents=[]
def getListStudentByScheduleId(Id):
    members=Attendances.query.filter_by(scheduleId=Id)
    result=[]
...
@blueprint.route('/attendance/schedule/<int:id>')
def attendance(id):
    global listStudents
    listStudents=getListStudentByScheduleId(id)
    print(listStudents.count(2))
    print('haha')
       #return redirect('/index')
    return render_template('/face_recognition/face.html',scheduleId=id)
