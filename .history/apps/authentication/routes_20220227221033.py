    ...
    while camera.isOpened():
        ...
        if ret:
            ...
            for (x, y, l, a) in detected_faces:
                ...
                #cv2.rectangle(img, (x, y), (x + l, y + a), (0, 0, 255), cv2.FILLED)
                font = cv2.FONT_HERSHEY_DUPLEX
                cv2.putText(img, name , (y0 + 6, x0 +25), font, 1.0, (150, 150, 150), 1)
                cv2.putText(img, str(round(distance_test,4)) , (y0 + 6, x0 -10), font, 1.0, (150, 0, 150), 1)
            for i in stu_old:
                if i not in stu_new:
                    addInfo_Out(i)
            stu_old=stu_new
