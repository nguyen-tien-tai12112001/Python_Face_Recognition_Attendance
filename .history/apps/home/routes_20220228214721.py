...
def add_schedule():
    ...
    if'add_schedule' in request.form:
        schedule = Schedule(**request.form)
        db.session.add(schedule)
        db.session.commit()
        return render_template('home/schedule/add-schedule.html', segment=segment)
    listGv=Teachers.query.all()
    listCl=Class.query.all()
    listSub=Subject.query.all()
    return render_template('home/schedule/add-schedule.html', segment=segment,listGv=listGv,listCl=listCl,listSub=listSub)
