...
def list_class():
    segment = get_segment(request)
    return render_template('home/list-class.html', rows=Class.query.all(), segment=segment)
# Subject
@blueprint.route('/list-subject.html', methods=['GET', 'POST'])
@login_required
def list_subject():
    segment = get_segment(request)
    return render_template('home/list-subject.html', rows=Subject.query.all(), segment=segment)
