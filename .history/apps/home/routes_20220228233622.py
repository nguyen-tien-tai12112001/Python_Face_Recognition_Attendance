...
class Summary(db.Model):
    id=db.Column(db.Integer,primary_key=True)
    countJoin=db.Column(db.Interger,primary_key=False)
    className= db.Column(db.String(50))
...
def summary_study():
    segment = get_segment(request)
    a=db.session.query(Attendance.scheduleId,func.count(Attendance.scheduleId)).group_by(Attendance.scheduleId).all()
