...
def summary_study():
    ...
    totals=db.session.query(Schedule.classId,Class.name,Attendance.msv,func.count(Attendance.msv)).join(Schedule).join(Class).group_by(Schedule.classId,Class.name)
    #totals=totals.join(Class).all()
    for i in totals:
     for a in i:
         print(a)
