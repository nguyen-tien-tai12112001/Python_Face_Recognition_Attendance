...
from apps.authentication.models import InfoClass, Schedule, Students, Subject, Teacher,Users, Attendance,SummaryStudy
...
def list_teacher():
    segment = get_segment(request)
    return render_template('home/teacher/list-teacher.html', rows=Subject.query.all(), segment=segment)
...
class ItemSchedule(db.Model):
    ...
    def __init__(self,classId,className,subjectName,teacheName,dateStart,dateEnd):
...
def list_schedule():
    ...
    for event in schedules:
        ...
        itemTeacher=Teacher.query.filter_by(mgv=event.mgv).first()
        itemSchedule= ItemSchedule(classId=itemClass.id,className=itemClass.name,subjectName=itemSubject.name,teacheName=itemTeacher.fullname,dateStart=event.startDate,dateEnd=event.endDate)
        items.append(itemSchedule)

        


    return render_template('home/schedule/list-schedule.html', rows=Schedule.query.all(), segment=segment)
